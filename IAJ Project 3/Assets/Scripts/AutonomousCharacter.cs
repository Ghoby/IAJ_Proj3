...
namespace Assets.Scripts
{
	public class AutonomousCharacter : MonoBehaviour
	{
		...
		public MCTSBiasedPlayout MCTSDecisionMaking { get; set; }
        //public MCTSRave MCTSDecisionMaking { get; set; }
		...
		private PathSmoothing pathSmooth = new PathSmoothing();
		...
		void Update()
		{
			...
			if (this.AStarPathFinding.InProgress)
			{
				...
				if (finished && this.currentSolution != null)
				{
					...
					//this.StraightLineSmoothing();
					this.currentSmoothedSolution = pathSmooth.SmoothPath(this.currentSolution, startPosition);
		...
		private void UpdateDLMCTS()
		{
			...
			this.TotalProcessingTimeText.text = "Max Playout Depth: " + this.MCTSDecisionMaking.MaxPlayoutDepthReached;
			this.BestDiscontentmentText.text = "Sum Process. Time: " + this.MCTSDecisionMaking.SumProcessingTime.ToString("F");
			this.ProcessedActionsText.text = "Max Selection Depth: " + this.MCTSDecisionMaking.MaxSelectionDepthReached;
			...
			{
				var actionText = "";
				if (this.MCTSDecisionMaking.BestActionSequence.Count != 0)
				{
		...
		/* public void StraightLineSmoothing()
		 {

			 for (int i = 0; i < this.currentSolution.PathPositions.Count - 2; i++)
			 {
				 if (IsWalkable(this.currentSolution.PathPositions[i], this.currentSolution.PathPositions[i + 2]))
				 {
					 //this.currentSolution.PathPositions.RemoveAt(i + 1);
					 this.currentSmoothedSolution.PathPositions.Add(this.currentSolution.PathPositions[i]);
					 this.currentSmoothedSolution.PathPositions.Add(this.currentSolution.PathPositions[i + 2]);
					 i++;
				 }
				 else
				 {
					 this.currentSmoothedSolution.PathPositions.Add(this.currentSolution.PathPositions[i]);
					 this.currentSmoothedSolution.PathPositions.Add(this.currentSolution.PathPositions[i + 1]);
					 this.currentSmoothedSolution.PathPositions.Add(this.currentSolution.PathPositions[i + 2]);
				 }
			 }
		 }

		 public bool IsWalkable(Vector3 p1, Vector3 p2)
		 {
